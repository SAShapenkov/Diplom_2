...
class TestOrderOptions:
    ...
    def test_create_order_unauthorized_user(self, create_order_payload):
    ...
    def test_create_order_token_wrong_ingredients_id(self, create_order_payload, create_user_payload,
                                                     make_user):
        user_payload = create_user_payload(email='rand', password='rand', name='rand')
        user = make_user(data=user_payload)
        token = user["text"]['accessToken']
